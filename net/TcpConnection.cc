...
void TcpConnection::HandleRead(Timestamp readTime)
{
    ...
    this->_eventLoop->assertInLoopThread();
    ...
    if (!_channel->IsWriteEnabled()) {
        ...
        if (writeLen == (ssize_t)len) {
            ...
            if (this->_messageWriteCompleteCallback) {
                ...
                _eventLoop->queueInLoopThread(std::bind(_messageWriteCompleteCallback, shared_from_this()));
            }
...
void TcpConnection::HandleWrite()
{
    ...
    if (actualWriteSize == (ssize_t)bufRemainingSize) {  // 写缓存全部发送完了
        ...
        if (this->_messageWriteCompleteCallback) {
            ...
            _eventLoop->queueInLoopThread(bind(this->_messageWriteCompleteCallback, shared_from_this()));
        }
...
{
    SetState(kDisconnecting);
    _eventLoop->queueInLoopThread(bind(&TcpConnection::ForceCloseInLoop, this));
...
{
    _eventLoop->assertInLoopThread();
    // _eventLoop->queueInLoopThread(bind(&TcpConnection::HandleClose, this)); BUG
...
{
    _eventLoop->queueInLoopThread(bind(&TcpConnection::ConnectionEstablishedInLoop, this));
}
